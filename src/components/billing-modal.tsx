...
export function BillingModal({ booking, products, readOnly, onClose, onCompleted }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-200 dark:border-neutral-800">
          <h2 className="font-bold">Abrechnung erfassen</h2>
          <button onClick={onClose} className="text-neutral-500 dark:text-neutral-400">
            ✕
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-5">
          <div className="text-xs font-mono text-neutral-600 dark:text-neutral-500 mb-4">
            #{booking.id} · {booking.room} · {booking.date} {booking.time_from?.slice(0, 5)}–
            {booking.time_to?.slice(0, 5)} · KST: {booking.costcenter}
          </div>

          <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-2 text-xs uppercase text-neutral-600 dark:text-neutral-500 border-b border-neutral-200 dark:border-neutral-800 pb-2 mb-2">
            <div>Produkt</div>
            <div>Preis</div>
            <div>Bestellt</div>
            <div>Verbraucht</div>
          ...
          {lines.length === 0 ? (
            <div className="text-neutral-400 dark:text-neutral-600 text-sm py-4">Keine Produkte.</div>
          ) : (
            lines.map(({ ordered, product }) => {
              ...
              return (
                <div
                  key={ordered.id}
                  className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-2 items-center py-2 border-b border-neutral-100 dark:border-neutral-900 text-sm"
                >
                  ...
                  <div className="font-mono text-xs text-lime-700 dark:text-lime-400">
          ...
          <div className="flex justify-between items-center mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-800">
            <span className="text-sm">Gesamtbetrag:</span>
            <span className="font-bold text-lime-700 dark:text-lime-400">{total.toFixed(2).replace('.', ',')} €</span>
          </div>
        </div>
