...
export function ServiceConsumptionModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-neutral-900 border border-neutral-700 rounded w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {lines.length === 0 ? (
            ...
            <div className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-2 text-xs uppercase text-neutral-500 border-b border-neutral-800 pb-2 mb-2">
              <div>Produkt</div>
              <div>Bestellt</div>
              <div>Verbraucht</div>
              <div>Rücklauf</div>
          ...
          {lines.map(({ ordered, product }) => {
            ...
            return (
              <div
                key={ordered.id}
                className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-2 items-center py-2 border-b border-neutral-900 text-sm"
              >
                <div>
                  <div>{product!.name}</div>
                  <div className="text-xs text-neutral-500 font-mono">{product!.unit}</div>
                </div>
                ...
                <div className="font-mono text-xs text-lime-400">{ordered.qty - c}</div>
              </div>
            )
          })}
        </div>
